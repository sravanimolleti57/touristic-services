 ...
 *   key   = "YYYY-MM-DD"
 *   value = "full"    → red dot (hotels/flights fully booked)
 *           "holiday" → green dot (special availability / holiday)
 */
    ...
    >
      {!compact && (
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 14, marginBottom: 18 }}>
          <div>
            ...
            <h3 style={{ margin: "10px 0 6px", fontSize: 24, lineHeight: 1.1 }}>Plan your next escape</h3>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: compact ? 10 : 16 }}>
        <button
          ...
          style={{ ...iconButtonStyle, width: compact ? 28 : 42, height: compact ? 28 : 42, borderRadius: compact ? 8 : 14 }}
        >
          <FaChevronLeft size={compact ? 10 : 14} />
        </button>

        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: compact ? 14 : 20, fontWeight: 800 }}>{formatMonthLabel(viewDate)}</div>
          {!compact && <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 2 }}>Tap a date to start planning</div>}
        </div>
        ...
        <button
          ...
          style={{ ...iconButtonStyle, width: compact ? 28 : 42, height: compact ? 28 : 42, borderRadius: compact ? 8 : 14 }}
        >
          <FaChevronRight size={compact ? 10 : 14} />
        </button>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: compact ? 8 : 12, justifyContent: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#ef4444", boxShadow: "0 0 5px rgba(239,68,68,0.7)", display: "inline-block" }} />
          <span style={{ fontSize: 10, color: "#94a3b8", fontWeight: 600 }}>Fully Booked</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#22c55e", boxShadow: "0 0 5px rgba(34,197,94,0.7)", display: "inline-block" }} />
          <span style={{ fontSize: 10, color: "#94a3b8", fontWeight: 600 }}>Holiday / Open</span>
        </div>
      </div>

      {/* Calendar grid */}
      <div style={{ ...calendarGridStyle, gap: compact ? 3 : 8 }}>
        {WEEKDAYS.map((day) => (
        ...
        {monthCells.map((cell) => {
          if (cell.empty) {
            return <div key={cell.key} style={{ ...emptyCellStyle, minHeight: compact ? 34 : 54 }} />;
          }

          const isSelected = toDateKey(cell.date) === toDateKey(selectedDate);
          const dateKey = toDateKey(cell.date);
          const availability = availabilityData[dateKey]; // "full" | "holiday" | undefined
          const isHovered = hoveredKey === dateKey;

          // dot colours
          const dotColor = availability === "full" ? "#ef4444"
            : availability === "holiday" ? "#22c55e"
            : null;
          const dotGlow = availability === "full" ? "rgba(239,68,68,0.8)"
            : availability === "holiday" ? "rgba(34,197,94,0.8)"
            : null;
            ...
            <button
              ...
              title={
                availability === "full"
                  ...
                  ? "✅ Holiday — great availability for hotels & flights"
                  : undefined
              }
              ...
              style={{
                ...
                boxShadow: isSelected
                  ...
                  : availability === "full" && isHovered
                  ? "0 4px 16px rgba(239,68,68,0.3)"
                  : availability === "holiday" && isHovered
                  ? "0 4px 16px rgba(34,197,94,0.25)"
                  : "none",
                transform: isHovered && !isSelected ? "scale(1.07)" : "scale(1)",
              }}
            >
              <span style={{ fontSize: compact ? 11 : 15, fontWeight: 700 }}>{cell.day}</span>
              ...
              {/* Today label — non-compact only */}
              {cell.today && !isSelected && !compact && !dotColor && (
                <span style={{ fontSize: 9, color: "#93c5fd", marginTop: 2 }}>Today</span>
              )}
            </button>
          );
        })}
      ...
      {/* Tooltip-style status — shows on hover for marked dates */}
      {!compact && (
